...
$(BOOTLOADER): $(SRC)/stage1.asm $(SRC)/stage2.asm $(SRC)/stage3.c $(LIBC)
	...
	$(LD) $(LDFLAGS) -T$(SRC)/stage3.ld --oformat binary -o $(BIN)/stage3.bin $(BIN)/stage3.o $(LIBC)
	rm $(BIN)/stage3.o
...
$(KERNEL): $(SRC)/kernel.c $(LIBC) #TODO: $(LIBK)
	...
	$(LD) $(LDFLAGS) -T$(SRC)/kernel.ld --oformat binary -o $(BIN)/kernel.bin $(BIN)/kernel.o $(LIBC) #TODO: LIBK
	rm $(BIN)/kernel.o

# This target compiles *.c (except kernel.c)
$(C_FILES): $(LIBC)
	@echo "Compiling $(SRC)/$@.c..."
	$(CC) $(CCFLAGS) -I$(LIBC_INC) -o $(BIN)/$@.o -c $(SRC)/$@.c 
	$(LD) $(LDFLAGS) -T$(SRC)/$@.ld --oformat binary -o $(BIN)/$@.bin $(BIN)/$@.o $(LIBC)
	rm $(BIN)/$@.o

$(LIBC): $(LIBC_DIR)/**.c
	@echo "Compiling $(LIBC)..."
	mkdir -p $(BIN)/libc
	for f in libc/*.c; do $(CC) $(CCFLAGS) -I$(LIBC_INC) -o $(BIN)/$$f.o -c $$f; done
