...
int vmm_init(void) {
	...
	uint32_t bitmap_loc = 0xC6000000;

	size_t page_count = dceil(vmm_bitmap.bytes, PAGE_SIZE);
	for (size_t i = 0; i < page_count; i++) {
		void* fr = pmm_alloc();
		if (fr == NULL) return 1;
		pag_map(bitmap_loc + i * PAGE_SIZE, (uint32_t)fr, 0);
	}

	// Set all pages as
	vmm_bitmap.map = (void*)bitmap_loc;
	bmap_set_all(&vmm_bitmap, 0);
...
void* vmm_alloc(size_t pages) {
	size_t idx = bmap_get_area(&vmm_bitmap, pages, 0);
	if (idx == BMAP_NOT_FOUND) return NULL;
	void* ptr = bmap_ptr(&vmm_bitmap, idx);

	bmap_set_area(&vmm_bitmap, idx * BLOCK_SIZE, pages * PAGE_SIZE, 1);

	for (size_t i = 0; i < pages; i++) {
		void* fr = pmm_alloc();
		if (fr == NULL) return NULL;
		pag_map((uint32_t)ptr + i * PAGE_SIZE, (uint32_t)fr, 0);
	}
...
void vmm_free(void* ptr, size_t pages) {
	for (size_t i = 0; i < pages; i++) {
		...
		uint32_t paddr = (uint32_t)pag_virt_to_phys((uint32_t)ptr);
	...
	bmap_set_area(&vmm_bitmap, base, pages, 0);
}
