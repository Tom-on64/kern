...
start:
    ...
    ; Some BIOSes will override the FAT BPB area,
    ; so we just fill it out to avoid pottentially
    ; overriding bootloader code.
    ; Source: https://github.com/limine-bootloader/limine/blob/trunk/stage1/hdd/bootsect.asm
.bpb:
    times 3-($-$$) db 0
    .bpb_oem_id:            db "KBOOT   "
...
.load_stage2:
    push 0x7000
    pop es      ; ES = 0x7000
    xor bx, bx  ; ES:BX = 0x70000
    mov eax, 512
    xor ebp, ebp
    mov ecx, 512
    call read_sectors
    jc err.4    ; CF was set -> failed to read from disk
